...
models = {
    ...
    "ours_checkcube": models.CLIC_based_codecs.RDT_CheckerCube,
    'ours_meanscalehyper_groupvit': models.ours_vit.MeanScaleHyperpriorGroupViT,
    'ours_channelar_groupvit': models.ours_vit.ChannelAutoRegressiveGroupViT,
    'ours_swin_hyper': models.ours_vit.TransformerBasedTransformCodingHyper,
    'ours_swin_channelar': models.ours_vit.TransformerBasedTransformCodingChAR,
    'ours_groupswin_hyper': models.ours_vit.GroupTransformerBasedTransformCodingHyper,
    'ours_groupswin_channelar': models.ours_vit.GroupChARTTC,   # 跑的是这个
    'ours_groupswin_TfChARM': models.ours_vit.GroupTfChARM, # transformer-based Channel Autoregressive Model
}

coco_map_dict = load_coco_labels()  # 加载 COCO 数据集的类别标签映射
...
parser.add_argument('--save', '-s', default='./logs/default/git_ssic/debug', type=str, help='directory for saving')
...
# codec setting
parser.add_argument("--model", type=str, default="ours_groupswin_channelar", help="Model")
...
parser.add_argument('--resume', type=str, default=NotImplementedError,
...
class SemanticStructuredImageCoding():
    def __init__(self):
        self.build_logger()
        # build codec
        self.codec = self.build_codec() # build learnable codec
    ...
    def build_learned_based_codec(self):
        if 'ours' in args.model:
            ...
            else:   # args.model:ours_groupswin_channelar
                model = models[args.model](args.transform_channels, args.hyper_channels)    # this
    ...
    def compress(self, input_p, bin_p, group_msk=None, instances_list=None):
        ...
        bs_header = self.enc_header(
            ...
            instances_list, args.ss_w_bboxes_flag, args.texture_codec_idx)  # args.texture_codec_idx=0
        ...
        with open(str(bin_p), "wb") as f:
            f.write(byte_stream)    # 将byte_stream写入bin_p

        with open(str(bin_p), "rb") as f:
            string = f.read()       # 读取byte_stream，之后计算bpp
            f.close()
    ...
    def decompress(self, bin_p, rec_p, groups_tobe_decode=None):
        ...
        # * decompress texture.
        shape = (header['hx'], header['wx'])    # shape表示什么？
        if args.ss_enabled_flag:
    ...
    def enc_header(
        ...
        # - texture codec
        bs_texture_codec_idx = pack_uints((texture_codec_idx, ))    # 0
        ...
        if ss_enabled_flag:
            ...
            ss_block_size = args.ss_block_size  # default=64
            ...
            group_msk = torch.nn.functional.interpolate(
                group_msk, scale_factor=1/group_msk_scale, mode='nearest')  # scale group_msk to 0.25 scale
            assert len(group_msk.unique()) < self.max_group_numbers # 64
    ...
    def get_group_msk(self, h, w, instances_list, mode='bbox'):
        ...
        if mode == 'bbox':
            # generate msks of each instance
            for instance in instances_list:
                x1, y1, x2, y2 = instance['bbox']
                if args.ss_expand_bbox_factor:  # default=0, expand the bounding box to generate group mask
                ...
                if args.ss_det_human: 
                    if instance['category_id'] != 1:
                        continue
                # set the region of bbox to 1
                msk_objects[int(y1):int(y2), int(x1):int(x2)] = 1
                # 将x/y坐标量化到ss_block_size的整数倍
        ...
        if len(msks) > 0:
            ...
            # Process the msk: If the block contains more than 1 pixels that belong to an object.
            # Set it as 1.
            msks = msks.sum(dim=0)  # 合并成一个msk
            ...
            # 对每个块内的像素求和，以计算每个块是否包含object像素，有则在new_msk中置为1
            msks = msks.sum(dim=-1)
            new_msk = msks
            new_msk[new_msk!=0] = 1

            new_msk = new_msk.numpy().astype(np.uint8)*255  # 可能是为了便于可视化msk

            # generate the connected components
            # num_labels：new_msk中连通组件数量，
            # group_msk对new_msk中每个连通组件标记为唯一的整数值，stats包含其他统计信息，如bbox
            ...
            # 将group_msk调整为(w_group_msk, h_group_msk)的大小
        ...
        # 未检测到instance的情况？
            ...
            if args.ss_det_human:
                ...
                group_msk = np.zeros((h_group_msk, w_group_msk), dtype=np.uint8)
                # 分别对应 new_msk全1, group_msk全0, object_msks全1
                return np.ones((h, w), dtype=np.uint8), group_msk, np.ones((h, w), dtype=np.uint8)
                
class SemanticStructuredFeatureCoding():
    def __init__(self):
        super().__init__()
