...
def ms_ssim(X, Y, win_size=11, win_sigma=1.5, win=None, data_range=255, size_average=True, normalize=False, full=False, weights=None):
    ...
    for _ in range(levels):
        ...
        size = min(win_size, X.shape[2], X.shape[3])
        # Scale down sigma if a smaller filter size is used.
        if size < win_size:
            win_sigma = size * win_sigma / win_size
            win = _fspecial_gauss_1d(size, win_sigma)
            win = win.repeat(X.shape[1], 1, 1, 1)
