...
class data_prefetcher():
    ...
    def preload(self):
        ...
        with torch.cuda.stream(self.stream):
            if not isinstance(self.next_input, list):
                self.next_input = [self.next_input]
            self.next_input = [next_input.to(self.device, non_blocking=True)
                               for next_input in self.next_input]
...
class VimeoDataset(data.Dataset):
    def __init__(self, root_dir, frame_index, crop_size, color_space='rgb'):
        self.root_dir = root_dir
        self.env = lmdb.open(root_dir,
                             max_readers=1,
                             readonly=True,
                             lock=False,
                             readahead=False,
...
class YUVDataset(data.Dataset):
    def __init__(self, root_dir, frame_index, crop_size, to_yuv444=True):
        self.root_dir = root_dir
        # print(root_dir)
        self.env = lmdb.open(root_dir,
                             max_readers=1,
...
if __name__ == '__main__':
    # root_dir = '/data/gaoyx/dataset/CDVL_lmdb'
    root_dir = '/gdata2/fengrs/dataset/CDVL_lmdb'
